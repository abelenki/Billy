...
class Controller( webapp.RequestHandler ):
    ...
    def inviteAction( self, key ):
        ...
        if mail.is_email_valid( email ):
            ...
            body = "%s wants you to join Billy:\n\n%s\nClick on this link to join:\n%s" % (
                    user.nickname(),
                    description,
    ...
    def accountsAction( self, key = None ):
        ...
        template_values['invites'] = db.Query(AccountInvite).filter('email =', user.email() );

        path = os.path.join(os.path.dirname(__file__), '../template/user/accounts.html')
    ...
    def accountAction( self, key ):
        ...
        path = os.path.join(os.path.dirname(__file__), '../template/user/account.html')
        template_values['content'] = template.render( path, template_values )
